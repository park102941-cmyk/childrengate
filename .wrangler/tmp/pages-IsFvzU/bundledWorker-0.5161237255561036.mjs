...
var V = /* @__PURE__ */ __name((e, t, r) => (r = e != null ? re(oe(e)) : {}, de(t || !e || !e.__esModule ? H(r, "default", { value: e, enumerable: true }) : r, e)), "V");
var x;
var c = E(() => {
  x = { collectedLocales: [] };
});
...
async function w(e, { request: t, assetsFetcher: r, ctx: a }, { path: n, searchParams: o }) {
  ...
  try {
    switch (e?.type) {
      ...
      case "override": {
        s = v(await r.fetch(L(d, e.path ?? n))), e.headers && f(s.headers, e.headers);
        break;
      }
      case "static": {
        s = await r.fetch(L(d, n));
        break;
      }
      default:
...
async function J(e) {
  ...
  try {
    ...
    switch (e.method) {
      case "GET": {
        let o = z(e, xe), s = await a.get(n, { softTags: o });
...
var k = class {
  ...
  applyLocaleRedirects(t) {
    ...
    let { locale: { redirect: a, cookie: n } } = t, o = n && this.cookies[n], s = A(o ?? ""), i = A(this.reqCtx.request.headers.get("accept-language") ?? ""), p = [...s, ...i].map((y) => a[y]).filter(Boolean)[0];
    if (p) {
  ...
  getLocaleFriendlyRoute(t, r) {
    return !this.locales || r !== "miss" ? t : K(t.src, this.locales) ? { ...t, src: t.src.replace(/\/\(\.\*\)\$$/, "(?:/(.*))?$") } : t;
  }
...
var va = { async fetch(e, t, r) {
  ...
  return n.run({ ...t, NODE_ENV: "production", SUSPENSE_CACHE_URL: b }, async () => o.run({ env: t, ctx: r, cf: e.cf }, async () => {
    if (new URL(e.url).pathname.startsWith("/_next/image")) return G(e, { buildOutput: m, assetsFetcher: t.ASSETS, imagesConfig: _.images });
    let i = B(e);
 ...
 * MIT Licensed
 */
//# sourceMappingURL=bundledWorker-0.5161237255561036.mjs.map
